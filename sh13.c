...
char gbuffer[256]; // buffer contient msg réseau reçu par le client
char gServerIpAddress[256]; // identifier informations serveur principale
int gServerPort; // identifier informations serveur principale (vérifier localhost 32000)
char gClientIpAddress[256]; // nom machine client
int gClientPort; // port machine client
char gName[256]; // nom connu par le sh13 "bob" par exemple
char gNames[4][256]; // nom des 4 joueurs dans l'ordre des connexions
int gId; // id envoyé par le serveur // tres important
int joueurSel; 
int objetSel;
int guiltSel;
int guiltGuess[13];
int tableCartes[4][8]; // table carte du joueur (propagé par le serveur ?)
int b[3]; // numero bitmap pour afficher les 3 cartes que l'on connait (en main) // tres important
int goEnabled; // bouton go visible
int connectEnabled; // boutn connect visible
...
char com; int indice_ligne; int indice_colonne; int valeurTab; // variables pour sscanf 

volatile int synchro; // tres important (difficile)
// thread réseaud prévient le thread graphique avec synchro = 1 et avec gbuffer rempli
void *fn_serveur_tcp(void *arg) // fonction threadable -> serveur TCP 
{
       ...
       if (bind(sockfd, (struct sockaddr *) &serv_addr, sizeof(serv_addr)) < 0)
        ...
        while (1)
        {
                ...
                bzero(gbuffer,256);
                n = read(newsockfd,gbuffer,255); // on rempli le gbuffer
                if (n < 0)
                {
                        printf("read error\n");
                        exit(1);
                ...
                while (synchro); // partagée entre les threads, on attend que le thread graphique finit avec synchro = 0

     }
}
// envoyer msg au serveur principal
...
int main(int argc, char ** argv)
{
	...
	int id;
        ...
        gServerPort=atoi(argv[2]); // transformé en entier
	...
	for (i=0;i<4;i++) // cartes non connues
		for (j=0;j<8;j++)
			tableCartes[i][j]=-1;
   ...
   synchro=0;
   ret = pthread_create ( & thread_serveur_tcp_id, NULL, fn_serveur_tcp, NULL); // thread serveur

    while (!quit) // boucle évènements graphiques
    {
	if (SDL_PollEvent(&event))
	{
		//printf("un event\n");
			...
			case  SDL_MOUSEBUTTONDOWN:
				...
				//printf("mx=%d my=%d\n",mx,my);
				if ((mx<200) && (my<50) && (connectEnabled==1)) // si coordonnées dans rectangle d'origine (0,0)
				{
					sprintf(sendBuffer,"C %s %d %s",gClientIpAddress,gClientPort,gName); // envoie msg au serveur pour se connecter

					// RAJOUTER DU CODE ICI (OK)

					sendMessageToServer(gServerIpAddress,gServerPort,sendBuffer);

					connectEnabled=0; // on fait disparaitre le bouton
				}
				...
				else if ((mx>=500) && (mx<700) && (my>=350) && (my<450) && (goEnabled==1))
				{
					...
					else if ((objetSel!=-1) && (joueurSel==-1)) // demander à tout le monde : "est ce que vous avez ce symbole"
					{
					...
					else if ((objetSel!=-1) && (joueurSel!=-1)) // demander à un joueur combien il en a
					{
				...
				break;
			case  SDL_MOUSEMOTION: // rien ici
				SDL_GetMouseState( &mx, &my );
				break;
        	}
	}

        if (synchro==1) // =1 => on a reçu un msg réseau donc qqc d'intéressant dans gbuffer
        {
                printf("consomme |%s|\n",gbuffer);
		switch (gbuffer[0])
		{
			...
			case 'M':
				// RAJOUTER DU CODE ICI (OK)
				sscanf(gbuffer,"%c %d",&com,&joueurSel);
